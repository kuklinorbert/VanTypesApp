...
class ClassificationBloc
  ...
  Stream<ClassificationState> _eitherPredictionOrError(
  ...
  Stream<ClassificationState> _eitherImageOrError(
    Either<Failure, List> permissionOrDownload,
  ) async* {
    yield permissionOrDownload.fold(
      (failure) {
  ...
  Stream<ClassificationState> _eitherModelOrError(
  ...
  Stream<ClassificationState> _eitherStoragePermissionOrError(
    Either<Failure, PermissionStatus> permissionOrDownload,
  ) async* {
    yield permissionOrDownload.fold(
      (failure) {
        return PermissionDenied(message: _mapFailureToMessage(failure));
      },
      (permission) {
  ...
  Stream<ClassificationState> _eitherCameraPermissionOrError(
    Either<Failure, PermissionStatus> permissionOrDownload,
  ) async* {
    yield permissionOrDownload.fold(
      (failure) => ErrorState(message: _mapFailureToMessage(failure)),
